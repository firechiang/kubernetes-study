...
```bash
# 创建存放根证书目录
$ mkdir -p /home/cfssl/pki/apiserver

# 创建config配置文件（注意：证书的过期时间是87600小时）
$ cat > /home/cfssl/pki/apiserver/ca-config.json <<EOF
{
  "signing": {
    "default": {
      "expiry": "87600h"
...
# 创建csr配置文件
$ cat > /home/cfssl/pki/apiserver/ca-csr.json <<EOF
{
  "CN": "kubernetes",
  "key": {
    "algo": "rsa",
...
$ /home/cfssl/bin/cfssl gencert                     \
  -ca=/home/cfssl/pki/apiserver/ca.pem              \
  -ca-key=/home/cfssl/pki/apiserver/ca-key.pem      \
  -config=/home/cfssl/pki/apiserver/ca-config.json  \
...
#### 六、在集群的各个主节点上创建 [vi /etc/systemd/system/kube-apiserver.service] Api Service系统启动文件（注意：IP要修改成节点自己的，而且创建文件时，要删除注释，否则会报错）
```bash
[Unit]
