  ...
  template:
    ...
    spec:
      ...
      - name: springboot-demo
        ...
        readinessProbe:
          # 以发七http请求的方式进行健康检查 （注意：它有3种检查方式（命令，http，检查端口）建议使用http发方式验证）
          httpGet:
            # 请求地址
            path: /host
            # 端口
...
# 查看某一个pod的详细信息
# 注意查看是否有健康检查：Liveness: http-get http://:2019/host delay=10s timeout=5s period=10s #success=1 #failure=2
