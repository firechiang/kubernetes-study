### 部署网络插件 Calico
#### 一、创建[mkdir -p /etc/kubernetes/addons && vi /etc/kubernetes/addons/calico.yaml]部署Calico的主要配置文件
```bash
...
#### 二、创建[mkdir -p /etc/kubernetes/addons && vi /etc/kubernetes/addons/calico-rbac-kdd.yaml]部署Calico的主要配置文件
```bash
# Calico Version v3.1.3
# https://docs.projectcalico.org/v3.1/releases#v3.1.3
kind: ClusterRole
...
#### 三、创建网络插件 Calico（注意：-f是指定配置文件（配置文件是我们在上面两步就已经创建好了的））
```bash
$ chmod +x /etc/kubernetes/addons/calico-rbac-kdd.yaml /etc/kubernetes/addons/calico.yaml
