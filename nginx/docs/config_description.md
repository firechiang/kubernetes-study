...
http {
    ...
    # 代理转发时,上游服务器的超时时间（0表示不限制；注意：如果超时了会将请求再次转发到其它服务器上）
    #proxy_next_upstream_timeout 10s;
    
    # 代理转发时，当上游服务器失败时，最多重试其它服务器的次数（0表示不限制，会将所有的服务都试一次，直到有好的服务为止；1表示只重试转发其它服务1次）
    #proxy_next_upstream_tries 0;
    
    # 代理转发时，当上游服务器返回状态大于300时，是否将请求交给 error_page 处理
    #proxy_intercept_errors off;
    
    # error_page 对应请求
    #error_page /test/index.html;
	...
	# 注意：加了 deferred 表示延迟唤醒Nginx进程处理http请求（就是如果只是建立tcp连接是不唤醒Nginx进程，当发送http请求才会唤醒Nginx进程，优化Nginx利用）
